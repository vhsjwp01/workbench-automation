...
#                                        POST ops
# 20150915     Jason W. Plummer          Refactored code to autodetect TARGET
#                                        initialization in ${param}/system.desc.
...
# --steps                 - A list of operational steps to execute (OPTIONAL)
# --start_step            - The step to start processing (OPTIONAL)
...
USAGE="${USAGE}[ --steps                 <A list of steps to execute.                                  ]${USAGE_ENDLINE}"
USAGE="${USAGE}[                          Valid steps are:                                             ]${USAGE_ENDLINE}"
USAGE="${USAGE}[                             all config import prepare pre analyze catalog             ]${USAGE_ENDLINE}"
USAGE="${USAGE}[                             anomalies missing fileconvert rdbmsconvert                ]${USAGE_ENDLINE}"
USAGE="${USAGE}[                             cobolconvert trad_jcl post                    *OPTIONAL*> ]${USAGE_ENDLINE}"
USAGE="${USAGE}[ --start_step            <The initial step at which processing should begin.           ]${USAGE_ENDLINE}"
USAGE="${USAGE}[                          Valid steps are:                                             ]${USAGE_ENDLINE}"
USAGE="${USAGE}[                             config import prepare pre analyze catalog                 ]${USAGE_ENDLINE}"
USAGE="${USAGE}[                             anomalies missing fileconvert rdbmsconvert                ]${USAGE_ENDLINE}"
USAGE="${USAGE}[                             cobolconvert trad_jcl post                    *OPTIONAL*> ]${USAGE_ENDLINE}"
...
if [ ${exit_code} -eq ${SUCCESS} ]; then
    ...
    while (( "${#}"  )); do
        ...
        case "${key}" in
            ...
            --wb_workdir|--project_name|--input_targets|--wb_toolpath|--target_COBOL_compiler|--target_DB|--fatal_check|--error_check|--warning_check|--missing_check|--clean_dirs|--steps|--start_step)
...
if [ ${exit_code} -eq ${SUCCESS} ]; then
    ...
    if [ ${check_for_start_step} -gt 0 ]; then
        let run_this_step=0 

        if [ "${this_step}" = "${start_step}" ]; then
            let run_this_step=1 
            let check_for_start_step=0
            steps="all"
        fi
        ...
        let run_this_step=$(echo "${steps}" | ${my_egrep} -c "\b${this_step}\b|\ball\b")
    fi

    if [ ${run_this_step} -gt 0 ]; then
        echo "${this_step}" | ${my_tr} '[a-z]' '[A-Z]'
        processing_verb="${this_step}"
        param_dir="${PARAM}"
    ...
    if [ ${run_this_step} -gt 0 ]; then
        ...
        for target_dir in ${TARGETS} ; do
            ...
            for target_file in ${target_files} ; do
                ...
                # Only process files whose size is greater than 0
                if [ -s "${prepared_dir}/${uc_target_dir}/${target_file}" ]; then
    ...
    if [ ${run_this_step} -gt 0 ]; then
        ...
        for file_extension in ${file_extensions} ; do
            ...
            for this_target_file in ${target_files} ; do
                ...
                # Only process files whose size is greater than 0
                if [ -s "${source_code_dir}/${target_file}" ]; then
